...
3. 打开项目详情页 → 自动请求签到码 + 签退码 → 直接在页面上展示两张二维码
"""
...
(function(){
    ...
    // ===== 全局状态 =====
    window.__ustc = {
        qrSignIn: null,      // 签到二维码 base64
        qrSignOut: null,     // 签退二维码 base64
        itemId: null,
        appid: null,
        fetched: false,
    ...
    // 同时拦截 XHR（uni-app 的 $u.post 可能用 XHR）
            ...
            xhr.addEventListener('load', function() {
                try {
                    ...
                    if (data.success && data.message) {
                        console.log('[USTC-Proxy] [XHR]📷 截获二维码! 长度:', data.message.length);
                    }
                } catch(e) {}
            });
        }
    ...
    async function fetchQRCode(itemId, appid, label) {
        console.log(`[USTC-Proxy] 🔍 正在请求${label}二维码... itemId=${itemId}, appid=${appid}`);
        try {
            ...
            if (json.success && json.message) {
                console.log(`[USTC-Proxy] ✅ ${label}二维码获取成功! base64长度:`, json.message.length);
                return json.message;
            } else {
                console.warn(`[USTC-Proxy] ⚠️ ${label}二维码接口返回失败:`, json.message || json);
                return null;
            }
        } catch(e) {
            console.error(`[USTC-Proxy] ❌ ${label}二维码请求异常:`, e);
            return null;
        }
    }
    ...
    function showQRPanel(signInData, signOutData) {
        ...
        panel.innerHTML =
            ...
            '<p style="margin:0 0 16px;color:#888;font-size:13px;">长按图片保存</p>' +
        ...
        // 异步更新图片（如果还没加载完）
        if (!signInData) {
            setTimeout(function() { updateQRPanels(); }, 500);
        }
    }

    // 更新面板中的二维码图片
    function updateQRPanels() {
        if (!window.__ustc.overlayVisible) return;

        var container = document.getElementById('ustc-qr-panel');
        if (!container) return;

        var imgs = container.querySelectorAll('img');
        if (window.__ustc.qrSignIn && imgs[0]) {
            imgs[0].src = 'data:image/png;base64,' + window.__ustc.qrSignIn;
        }
        if (window.__ustc.qrSignOut && imgs[1]) {
            imgs[1].src = 'data:image/png;base64,' + window.__ustc.qrSignOut;
        }
    }

    // ===== ④ 主流程：Vue 补丁 + 自动获取二维码 =====
    function patchVueAndFetchQR() {
        var attempts = 0;
        var maxAttempts = 80;  // 最多等 8 秒
        var interval = setInterval(function() {
            ...
            if (target) {
                ...
                try {
                    ...
                    console.log('[USTC-Proxy] ✅✅ 按钮已强制显示');
                } catch(e) {
                ...
                if (!itemId) {
                    console.warn('[USTC-Proxy] ⚠️ 未找到 itemId，无法自动获取二维码');
                    return;
                }
                ...
                // --- 自动获取签到码 + 签退码 ---
                if (!window.__ustc.fetched) {
                    window.__ustc.fetched = true;
                    ...
                    setTimeout(function() {
                        Promise.all([
                            ...
                            if (results[0] || results[1]) {
                                ...
                                alert('USTC Proxy: 二维码获取失败，请检查网络连接或登录状态。');
                            }
                        });
                    }, 1500);  // 等 1.5 秒确保登录态和 cookie 就绪
                }
            ...
            if (attempts >= maxAttempts) {
                clearInterval(interval);
                console.warn('[USTC-Proxy] ⚠️ Vue 组件查找超时');
            }
        }, 100);
    }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', patchVueAndFetchQR);
    } else {
        patchVueAndFetchQR();
    }

    console.log('[USTC-Proxy] ✓ 所有客户端补丁已就绪，等待项目详情页加载...');
})();
...
<div class="card">
    <h1>🎓 USTC Young 代理</h1>
    <p class="subtitle">前端覆写 · 自动获取签到/签退二维码</p>
    ...
    <div class="section">
        ...
        <div class="info">
            1. 点击上方链接进入<br>
            2. 登录后进入任意<strong>项目详情页</strong><br>
            3. 页面将<strong>自动弹出签到码 + 签退码</strong><br>
            4. 长按图片即可保存<br><br>
            <span style="color:#d32f2f;">⚡ 无需手动点击任何按钮</span>
        </div>
    </div>
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="USTC Young 前端覆写代理 v3（纯标准库）")
